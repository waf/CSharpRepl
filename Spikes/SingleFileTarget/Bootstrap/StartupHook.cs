...
internal static class StartupHook
{
    public static void Initialize()
    {
        ...
        Console.WriteLine($"[hook] bootstrap Location = '{typeof(StartupHook).Assembly.Location}' (external, NOT bundled)");
    ...
    private static void StartInspector(string dir)
    {
        try
        {
            var engine = Spike5.Bootstrap.EngineHost.Load(dir);
            Spike5.Contracts.InspectorHost.Engine = engine;
            new System.Threading.Thread(() => Spike5.Bootstrap.EngineHost.RunInspectorDemo(engine))
            {
                IsBackground = true,
                Name = "inspector"
            }.Start();
