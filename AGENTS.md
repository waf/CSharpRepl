...
- **`NuGet.*` all resolve from the SDK at run time.** In `CSharpRepl.Services.csproj` *every* `NuGet.*` package is `PrivateAssets="all"`: the compile-time reference stays but the runtime DLL isn't shipped, so they bind to the SDK's bundled copies (same `MSBuildLocator` mechanism as the MSBL001 fix above — avoids version skew, and the reason `NugetPackageInstaller`'s NuGet types only load *after* `MSBuildLocator.RegisterDefaults()` has run). Because nothing under `NuGet.*` ships, the R2R exclude list in `CSharpRepl.csproj` (active under `-p:PackRidSpecific=true`) needs only `Microsoft.CodeAnalysis.Workspaces.MSBuild.dll` (also SDK-loaded). Note: before the restore-based rework, `NuGet.PackageManagement` (and the `NuGet.Resolver` it dragged in) *was* shipped — the SDK doesn't bundle it — leaving an incomplete reference closure that broke R2R crossgen2, so both `.dll`s also had to be R2R-excluded. That dependency is gone now. Don't reintroduce a shipped `NuGet.*` runtime assembly without re-checking MSBL001 and the R2R exclude list (verify with an R2R publish — see `Roslyn/References/AssemblyReferenceReadme.md` §4).
