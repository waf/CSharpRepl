...
internal static class Program
{
    ...
    internal static async Task<int> RunAsync(string[] args, IConsoleService? console = null, bool? inputRedirectedOverride = null)
    {
        ...
        if (config.EarlyExitPlainText is { } plainExit)
        {
            // Unwrapped, no Spectre rendering — machine-consumable (e.g. `inspect init` shell exports).
            console.WriteStandardOutputLine(plainExit);
        ...
        // references + the inspector globals, so completion/highlighting are target-aware (M5).
