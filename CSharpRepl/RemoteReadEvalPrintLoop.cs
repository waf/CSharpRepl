...
internal sealed class RemoteReadEvalPrintLoop
{
    ...
    public async Task RunAsync(Configuration config)
    {
        ...
        while (true)
        {
            ...
            if (command is "exit" or "#disconnect") { break; }
            ...
            // Ctrl+C aborts the controller process (mirrors the local loop); the in-flight round-trip is
            // abandoned rather than cancelled mid-protocol.
    ...
    private void PrintBanner(HandshakeMessage handshake)
    {
        ...
        console.WriteLine();
        console.WriteLine("Type C# to evaluate it in the target. Type #disconnect to detach (the target keeps running), or exit to quit.");
        console.WriteLine(string.Empty);
    }
    ...
    private void PrintHelp()
    {
        console.Write(new Markup(
            ...
            Commands:
              - [green]#disconnect[/]: detach and leave the target running.
              - [green]exit[/]: quit the REPL.
