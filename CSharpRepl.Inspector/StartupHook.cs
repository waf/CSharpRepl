...
/// must honor: the bootstrap's only non-framework dependency (CSharpRepl.Inspector.Contracts) lives in the
...
internal static class StartupHook
{
    public static void Initialize()
    {
        try
        {
            ...
            AssemblyLoadContext.Default.Resolving += (context, name) =>
            {
                ...
                if (name.Name is null || !name.Name.StartsWith("CSharpRepl.Inspector", StringComparison.Ordinal))
                    return null;
    ...
    {
        var engine = CSharpRepl.Inspector.EngineHost.Load(bootstrapDirectory);
        CSharpRepl.Inspector.InspectorServer.StartInBackground(engine);
    }
}
